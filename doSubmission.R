...
nam = names(data)
omitClass <- c(which(nam=="customer_ID"),which(nam=="last"),which(nam=="targetA"),
               which(nam=="targetB"),which(nam=="targetC"),which(nam=="targetD"),
...
for(fold in 3:folds)
{
  ...
  { 
    ...
    models_options[[option]][[fold]] = gbm(as.formula(paste(nam[targetOptions[option]],'~.')),
                                           ...
                                           distribution="multinomial",keep.data=F,verbose=F,n.cores=1,
